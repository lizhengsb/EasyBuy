...
<body>
<form name="form" action="MproductServlet?action=selectAll-product" method="post">
		<table align="right">
			<tr>
				<td>总共${plist.totalPage}页</td>
				<td></td>
				...
				<td>当前第<select name="nowPage" id="sel"
					onchange="gopage(this.value)">
						<c:forEach begin="1" end="${plist.totalPage}" var="p">
	...
	<script>
		var p=document.getElementById("sel").value = ${plist.nowPage};
		if(p=="1"){
			var obj=document.getElementById("before");
			obj.style.display="none";
		}else if(p=="${plist.totalPage}"){
