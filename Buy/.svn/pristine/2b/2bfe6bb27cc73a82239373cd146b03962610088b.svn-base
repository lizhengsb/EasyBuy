...
<body>
<form name="form" action="ManageServlet?action=selectAll-user" method="post">
		<table align="right">
			<tr>
				<td>总共${muser.totalPage}页</td>
				<td></td>
				...
				<td>当前第<select name="nowPage" id="sel"
					onchange="gopage(this.value)">
						<c:forEach begin="1" end="${muser.totalPage}" var="p">
	...
	<script>
		var p=document.getElementById("sel").value = ${muser.nowPage};
		if(p=="1"){
			var obj=document.getElementById("before");
			obj.style.display="none";
		}else if(p=="${muser.totalPage}"){
