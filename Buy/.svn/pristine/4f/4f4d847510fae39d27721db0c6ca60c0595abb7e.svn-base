...
<body>
<form name="form" action="MorderServlet?action=selectAll-order" method="post">
		<table align="right">
			<tr>
				<td>总共${orderlist.totalPage}页</td>
				<td></td>
				...
				<td>当前第<select name="nowPage" id="sel"
					onchange="gopage(this.value)">
						<c:forEach begin="1" end="${orderlist.totalPage}" var="p">
	...
	<script>
		var p=document.getElementById("sel").value = ${orderlist.nowPage};
		if(p=="1"){
			var obj=document.getElementById("before");
			obj.style.display="none";
		}else if(p=="${orderlist.totalPage}"){
			var ob=document.getElementById("next");
			ob.style.display="none";
		}else if("1"=="${orderlist.totalPage}"){
			var obj=document.getElementById("before");
			obj.style.display="none";
			var ob=document.getElementById("next");
			ob.style.display="none";
